...
export function createThread(firstMessage: string): Thread {
  ...
  if (response.status !== 200) {
    throw new Error(`OpenAI API error: ${response.status.toString()} ${response.statusText}`);
  }
  
  const responseJson = JSON.parse<OpenAIResponse>(response.text());
  const title = JSON.parse<TitleResponse>(responseJson.choices[0].message.content).title;
  ...
  const threadResponse = postgresql.query<Thread>(dbName, threadQuery, threadParams);
...
export function getReply(threadId: string, userMessage: string): Message {
  ...
  const botMessageResponse = postgresql.query<Message>(dbName, botMessageQuery, botMessageParams);
